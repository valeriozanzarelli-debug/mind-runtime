...
from organism.sensory.object_scene import analyze_objects
...
class BabyAgent:
    def __init__(self, seed: int = 42, *, store_path: str | None = None) -> None:
        ...
        self.composer = SpeechComposer(seed=seed)
    ...
    def _act(
        ...
        *,
        ...
        if code_out:
            ...
            composed = self.composer.long_form(thought=thought, motor=self.speech)
        elif wants_voice:
        ...
        self._last_ws = ws_d
        self._consciousness_log.extend(stream)
    ...
    def brain_pulse_tick(self, *, persist: bool = False) -> dict[str, Any]:
        ...
        if self.affect._pulse_count % 4 == 0:
            pulse_stream = build_consciousness_stream(
                thought={"themes": [], "symbols": [], "pressure": round(self.affect.state.curiosity * 0.35, 3)},
                workspace=self._last_ws,
                self_state=self.self_model.state.to_dict(),
                wave=wave.to_dict(),
                emotion=self.affect.state.to_dict(),
                phase="pulse",
            )
            self._consciousness_log.extend(pulse_stream)
            self._consciousness_log = self._consciousness_log[-240:]
    ...
    def self_hear(self, *, text: str) -> dict[str, Any]:
        ...
        self._consciousness_log.append(f"sé: ho detto «{text[:48]}» (sim {err.get('similarity', 0):.0%})")
        self._consciousness_log = self._consciousness_log[-240:]
    ...
    def teach_from_web_image(
        ...
        *,
        ...
        """Insegna da immagine internet — oggetto, volto o emozione."""
        ...
        try:
            img = fetch_image_rgba(url, size=96)
        ...
        grid, sig = self._decode_vision(
            ...
            image_w=int(img.get("image_w", 96)),
            image_h=int(img.get("image_h", 96)),
    ...
    def health(self) -> dict[str, Any]:
        ...
        outputs: list[str] = []
        for q in probes:
            m = self.sense(text=q)["moment"]
            outputs.append(str(m.get("spoke", "")))
        unique_out = len({o.lower().strip() for o in outputs if o})
        diversity = unique_out / max(1, len(probes))
        return {
            "ok": org.brain.neuron_count > 1000 and lex.count >= 20,
            "neurons": org.brain.neuron_count,
            "synapses": org.brain.synapse_count,
            "synapses_grown": org.brain.synapse_count - self._synapses_at_birth,
            ...
            "probe_outputs": list(zip(probes, outputs)),
